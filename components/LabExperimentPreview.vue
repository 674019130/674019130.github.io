...
function dismiss(event: KeyboardEvent) {
  if (event.key === 'Escape' && (open.value || timer)) { suppressed = true; close() }
}
onMounted(() => {
...
<template>
  ...
  <Teleport v-if="open" to="body">
    <div class="timeline-hover-preview" :style="{ left: `${x}px`, top: `${y}px` }" aria-hidden="true" inert>
      <iframe :src="source" :title="`${props.title} preview`" tabindex="-1" scrolling="no" />
    </div>
  </Teleport>
</template>
